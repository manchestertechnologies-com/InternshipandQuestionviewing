...
assert(res1.includes('\\frac{1}{2}'), 'Regression 1: 1/2 stacked', res1);

const res2 = formatCleanText('3/4');
assert(res2.includes('\\frac{3}{4}'), 'Regression 2: 3/4 stacked', res2);

const res3 = formatCleanText('a/b');
assert(res3.includes('\\frac{a}{b}'), 'Regression 3: a/b stacked', res3);

const res4 = formatCleanText('x/y');
assert(res4.includes('\\frac{x}{y}'), 'Regression 4: x/y stacked', res4);

const res5 = formatCleanText('(x+y)/(a+b)');
assert(res5.includes('\\frac{x+y}{a+b}'), 'Regression 5: (x+y)/(a+b) stacked', res5);

const res6 = formatCleanText('(x²+y²)/(a²+b²)');
assert(res6.includes('\\frac{x²+y²}{a²+b²}'), 'Regression 6: (x²+y²)/(a²+b²) stacked', res6);

const res7 = formatCleanText('sinθ/cosθ');
assert(res7.includes('\\frac{sinθ}{cosθ}'), 'Regression 7: sinθ/cosθ stacked', res7);

const res8 = formatCleanText('Ca²⁺/SO₄²⁻');
assert(res8.includes('\\frac{Ca²⁺}{SO₄²⁻}'), 'Regression 8: Ca²⁺/SO₄²⁻ stacked', res8);

const res9 = formatCleanText('10⁻³/10⁻⁶');
assert(res9.includes('\\frac{10⁻³}{10⁻⁶}'), 'Regression 9: 10⁻³/10⁻⁶ stacked', res9);

const res10 = formatCleanText('((a+b)/(c+d))');
assert(res10.includes('\\frac{a+b}{c+d}'), 'Regression 10: ((a+b)/(c+d)) stacked without orphan parens', res10);
...
assert(latexRes.includes('\\frac{x2-x1}{y2-y1}'), 'Direct LaTeX Input: m=\\frac{x2-x1}{y2-y1} stacked', latexRes);

// 4. User Screenshot Case: m=x2-x1/y2-y1
const resUserScreen = formatCleanText('m=x2-x1/y2-y1');
assert(resUserScreen.includes('\\frac{x2-x1}{y2-y1}'), 'User Screenshot Formula: m=x2-x1/y2-y1 stacked', resUserScreen);
...
assert(wordRes.includes('\\frac{x+y}{a+b}'), 'Word Equation OMML fraction converted to stacked fraction', wordRes);
